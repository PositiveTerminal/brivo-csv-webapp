...
class WebAppTestCase(WebAppTestBase):
    ...
    def test_selftest(self, mock_get_brivo):
        mock_get_brivo.return_value.__aenter__.return_value.healthcheck.return_value = (
    ...
    def test_login(self, mock_get_brivo):
        mock_get_brivo.return_value.__aenter__.return_value.get_access_token.return_value = MagicMock(
            status=200
        )
    ...
    def test_remove_old_processed_files(self):
        def _create_n_files(n, atime, mtime):
            ...
            for i in range(n):
                path = os.path.join(
                    writable_path(PathNames.PROCESSED_DIR),
                    f"testfile{i}-{randint(0,1024)}.csv",
                )
